...
const SHEET_URL = 'https://script.google.com/macros/s/AKfycbwJxoMYFDgMHlcXof7MfV8xHPOSIF05dmppd_K0xznCsv9KIvWSMA7PYs-HEV5HO2ip/exec';
// ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
...
async function assignCondition() {
  ...
  try {
    const resp  = await fetch(SHEET_URL + '?action=getCount');
    const data  = await resp.json();
    const count = typeof data.count === 'number' ? data.count : 0;
    CONDITION   = count % 2 === 0 ? 'frequent' : 'delayed';
  } catch (e) {
    // Sheet unreachable — fall back to random so the page still works
    CONDITION = Math.random() < 0.5 ? 'frequent' : 'delayed';
    console.warn('Could not fetch condition counter, using random fallback:', e);
  }
  labelEl.textContent =
